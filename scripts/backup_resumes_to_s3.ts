/**
 * Pre-migration: backup ATS resumes (optimized_resume_md) to S3 (raw SQL)
 *
 * Fetches all jobs with optimized_resume_md, uploads each to S3,
 * and saves {jobId: s3Key} mappings to a JSON file.
 *
 ...
 * Usage: npx tsx scripts/backup_resumes_to_s3.ts
 */
import fs from 'fs';
import pg from 'pg';
...
const OUTPUT_FILE = 'scripts/resume_s3_keys.json';
...
async function main() {
  ...
  const countRes = await pgClient.query(
    `SELECT COUNT(*) FROM jobs WHERE optimized_resume_md IS NOT NULL`
  );
  const total = parseInt(countRes.rows[0].count, 10);
  console.log(`Found ${total} jobs with ATS resumes`);
  ...
  for (let offset = 0; offset < total; offset += BATCH_SIZE) {
    ...
    while (true) {
      try {
        res = await pgClient.query(
          `SELECT id, user_id, optimized_resume_md
           FROM jobs
           WHERE optimized_resume_md IS NOT NULL
           ORDER BY id
           LIMIT $1 OFFSET $2`,
          [BATCH_SIZE, offset]
        );
    ...
    await Promise.all(res.rows.map(row =>
      limit(async () => {
        if (!row.optimized_resume_md || !row.id || !row.user_id) {
        ...
        const key = `job-resumes/${row.user_id}/${row.id}.txt`;
        ...
        try {
          await s3.send(new PutObjectCommand({
            Bucket: bucketName,
            Key: key,
            Body: row.optimized_resume_md,
            ContentType: 'text/plain',
          }));
          mapping[row.id] = key;
          batchResults.uploaded.push(row.id);
        } catch (err) {
          console.error(`Failed to upload resume for job ${row.id}:`, err);
